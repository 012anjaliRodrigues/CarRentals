...
const TariffPopup: React.FC<{
  ...
  modelId?: string;
  modelName?: string;
}> = ({ ownerId, allModels, onClose, onSaved }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let existingMap: Record<string, { from: string; to: string; rate: string; notes: string }> = {};

      if (modelIds.length > 0) {
        const { data } = await supabase
          .from('tariffs')
          .select('model_id, rate_per_day, effective_from, effective_to, notes')
          .eq('owner_id', ownerId)
          .in('model_id', modelIds)
          .order('created_at', { ascending: false });

        ((data as any[]) || []).forEach((t: any) => {
          if (!existingMap[t.model_id]) {
            existingMap[t.model_id] = {
      ...
      const firstNotes = Object.values(existingMap)[0]?.notes || '';
      setNotes(firstNotes);

      setRows(allModels.map(m => ({
        modelId: m.modelId,
        name:    m.name,
        from:    existingMap[m.modelId]?.from || today,
        to:      existingMap[m.modelId]?.to   || '',
        rate:    existingMap[m.modelId]?.rate  || '',
      })));
      setLoading(false);
    };
    load();
  ...
  const updateRow = (modelId: string, field: keyof Omit<TariffRow, 'modelId' | 'name'>, value: string) => {
    setRows(prev => prev.map(r => r.modelId === modelId ? { ...r, [field]: value } : r));
  };

  const handleSave = async () => {
    const filled = rows.filter(r => r.rate.trim() !== '');
    if (filled.length === 0) { toast.error('Enter a rate for at least one model.'); return; }
    setSaving(true);
    try {
      for (const row of filled) {
        ...
        const { error } = await supabase.from('tariffs').insert({
          ...
          rate_per_day:      parseFloat(row.rate),
          base_rate_per_day: parseFloat(row.rate),
          effective_from:    row.from || null,
          effective_to:      row.to   || null,
          notes:             notes    || null,
        });
        if (error) { toast.error(`Failed for ${row.name}: ${error.message}`); continue; }
        await supabase.from('models').update({ base_rate_per_day: parseFloat(row.rate) }).eq('id', row.modelId);
      }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl z-10 flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-8 pt-8 pb-5 border-b border-[#d1d0eb]/30">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#6c7e96] font-medium mt-0.5">Set rates for all vehicle models at once</p>
            </div>
          </div>
        ...
        {/* Column headers */}
        <div className="grid grid-cols-12 px-8 py-3 text-[10px] font-bold text-[#6c7e96] tracking-widest uppercase border-b border-[#d1d0eb]/20 bg-[#F9F9FF]">
          <div className="col-span-4">Brand / Model</div>
          <div className="col-span-3">From</div>
          <div className="col-span-3">To</div>
          <div className="col-span-2">Rate / Day (₹)</div>
        </div>

        {/* Scrollable rows */}
        <div className="overflow-y-auto flex-1 divide-y divide-[#d1d0eb]/20">
          {loading ? (
            ...
            <div className="py-12 text-center text-sm text-[#6c7e96] font-medium">No vehicle models found. Add vehicles first.</div>
          ) : (
            rows.map(row => (
              <div key={row.modelId} className="grid grid-cols-12 items-center px-8 py-3.5 hover:bg-[#F9F9FF]/60 transition-colors">
                <div className="col-span-4 flex items-center space-x-2 pr-3">
                  <CarIcon size={13} className="text-[#6360DF] shrink-0 opacity-60" />
                  <span className="text-sm font-bold text-[#151a3c] truncate">{row.name}</span>
                </div>
                <div className="col-span-3 pr-2">
                  <input type="date" value={row.from} onChange={e => updateRow(row.modelId, 'from', e.target.value)}
                    className="w-full bg-[#F8F9FA] border border-[#d1d0eb] rounded-xl py-2 px-3 text-xs font-medium text-[#151a3c] outline-none focus:border-[#6360DF] focus:ring-2 focus:ring-[#6360DF]/10" />
                </div>
                <div className="col-span-3 pr-2">
                  <input type="date" value={row.to} min={row.from} onChange={e => updateRow(row.modelId, 'to', e.target.value)}
                    className="w-full bg-[#F8F9FA] border border-[#d1d0eb] rounded-xl py-2 px-3 text-xs font-medium text-[#151a3c] outline-none focus:border-[#6360DF] focus:ring-2 focus:ring-[#6360DF]/10" />
                </div>
                <div className="col-span-2">
                  <input type="number" value={row.rate} onChange={e => updateRow(row.modelId, 'rate', e.target.value)}
                    placeholder="2500" min="0"
                    className="w-full bg-[#F8F9FA] border border-[#d1d0eb] rounded-xl py-2 px-3 text-xs font-bold text-[#151a3c] outline-none focus:border-[#6360DF] focus:ring-2 focus:ring-[#6360DF]/10" />
                </div>
              </div>
            ))
          )}
              ...
              {blackoutTypes.map(bt => (
                <button key={bt.value} type="button" onClick={() => setType(bt.value)}
                  className={`py-2.5 px-4 rounded-xl text-sm font-bold border transition-all ${
                    type === bt.value ? 'bg-red-50 border-red-300 text-red-600' : 'bg-[#F8F9FA] border-[#d1d0eb] text-[#6c7e96] hover:border-red-200'
                  }`}>{bt.label}</button>
              ))}
            </div>
          </div>
...
const FleetListing: React.FC = () => {
  ...
  const loadFleet = async () => {
    ...
    const { data: tariffData } = await supabase
      .from('tariffs').select('model_id, rate_per_day, effective_from, effective_to')
