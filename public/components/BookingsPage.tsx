...
import { Search, Calendar, Plus, MapPin, Loader2 } from 'lucide-react';
...
const BookingsPage: React.FC = () => {
  ...
  // ── KEY FIX: CreateNewBooking now passes the real DB row ──
  const handleConfirmBooking = (bookingRow: any) => {
    loadBookings(); // refresh the list in background
    setSelectedBooking(bookingRow); // bookingRow is already the full DB row
    setView('confirmed');
  };
  ...
  const filtered = bookingData.filter(b => {
    ...
    const pickupDate  = b.raw?.pickup_at?.split('T')[0] || '';
    const dropDate    = b.raw?.drop_at?.split('T')[0]   || '';
    const matchDate   = !selectedDate || (pickupDate <= selectedDate && dropDate >= selectedDate);
      ...
      <AnimatePresence mode="wait">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-[#d1d0eb]/20">
                    {loading ? (
                      ...
                      <tr><td colSpan={7} className="py-16 text-center text-[#6c7e96] text-sm font-medium">
                        {bookingData.length === 0
                          ? 'No bookings yet. Click "New Booking" to get started.'
                          : 'No bookings match your search or selected date.'}
                      </td></tr>
                    ) : (
                      filtered.map(booking => (
                        <tr key={booking.id} onClick={() => handleRowClick(booking)}
                          className="group hover:bg-[#F8F9FA] transition-colors cursor-pointer">
                          <td className="py-5 pl-10 text-sm font-medium text-[#151a3c] whitespace-nowrap">{booking.date}</td>
                          ...
                          <td className="py-5 px-6 text-sm font-medium text-[#151a3c] whitespace-nowrap">{booking.bookingPeriod}</td>
                          <td className="py-5 px-6 whitespace-nowrap">
                            <div className="flex items-center space-x-2 text-[#6c7e96] text-sm font-medium">
                              <MapPin size={14} className="text-[#6360DF]" /><span>{booking.location}</span>
                            </div>
                          </td>
                          <td className="py-5 px-6 text-center">
                            <span className="inline-block bg-[#f1f5f9] px-2.5 py-1 rounded-md text-sm font-bold text-[#151a3c]">{booking.vehicles}</span>
                          </td>
                          <td className="py-5 px-6 text-sm font-extrabold text-[#151a3c]">{booking.total}</td>
        ...
        {view === 'confirmed' && selectedBooking && (
          <motion.div key="confirmed" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
            <BookingDetails booking={selectedBooking} mode="confirmed" onBack={() => setView('list')} onEdit={() => {}} />
          </motion.div>
        )}
