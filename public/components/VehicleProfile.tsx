...
import {
  ArrowLeft, Calendar, Bell, Plus, MoreVertical,
...
const VehicleProfile: React.FC<VehicleProfileProps> = ({ vehicle, vehicleId, instancePlate, status, onBack }) => {
  ...
  // ── Load bookings for this specific vehicle ───────────────
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          id,
          bookings (
            id, customer_name, pickup_at, drop_at, status
          )
        `)
        .eq('vehicle_id', vehicleId);

      if (error) { console.error(error); setBookingsLoading(false); return; }
  ...
  const handleAddReminder = async () => {
    ...
    try {
      ...
      if (diffDays < 0) dbStatus = 'Overdue';
      else if (diffDays <= 7) dbStatus = 'Due Soon';
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none flex items-center space-x-2 px-5 py-2.5 border border-[#d1d0eb] rounded-xl text-sm font-bold text-[#151a3c] hover:bg-slate-50 transition-colors bg-white outline-none pr-8 cursor-pointer"
                >
                  <option value="All Bookings">All Bookings</option>
                  <option value="UPCOMING">Upcoming</option>
                  <option value="COMPLETED">Completed</option>
                </select>
                <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#6c7e96] pointer-events-none" />
              </div>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-[#f8f9fc]">
                  {filteredBookings.map((booking) => (
                    <tr key={booking.id} className="group hover:bg-[#f8f9fc]/50 transition-colors">
                      ...
                      <td className="px-4 py-6">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-extrabold tracking-widest ${booking.status === 'UPCOMING' || booking.status === 'BOOKED' || booking.status === 'ONGOING' ? 'bg-[#EEEDFA] text-[#6360DF]' : 'bg-[#D1FAE5] text-[#065F46]'}`}>
                ...
                {reminders.map((reminder) => (
                  <div key={reminder.id} className="bg-white rounded-2xl p-5 shadow-sm border border-[#d1d0eb]/10 flex items-center justify-between">
                    <div className="space-y-1">
                      <p className="text-[9px] font-bold text-[#6c7e96] tracking-widest uppercase">{reminder.title}</p>
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <button
                  ...
                  disabled={!newReminder.date || isSavingReminder}
                ...
                >
                  {isSavingReminder ? <Loader2 size={18} className="animate-spin" /> : null}
