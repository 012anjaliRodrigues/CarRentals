...
import {
  Bell, Search, Plus, MoreVertical, CheckCircle2, AlertCircle,
  Clock, Shield, Wrench, DollarSign, Calendar, ChevronDown, Car, Loader2
} from 'lucide-react';
...
const RemindersPage: React.FC = () => {
  ...
  const [selectedReminder, setSelectedReminder] = useState<ReminderDB | null>(null);
  ...
  const loadReminders = async () => {
    ...
    const [remindersRes, vehiclesRes] = await Promise.all([
      ...
      supabase
        .from('vehicles')
        .select('id, registration_no, models(brand, name, categories(name))')
        .eq('owner_id', ownerRow.id),
    ]);

    const mapped: ReminderDB[] = ((remindersRes.data as any[]) || []).map(r => ({
      id: r.id,
      vehicle: r.vehicles?.registration_no || '—',
      model: r.vehicles?.models ? `${r.vehicles.models.brand} ${r.vehicles.models.name}` : '—',
    ...
    const vOptions: VehicleOption[] = ((vehiclesRes.data as any[]) || []).map(v => ({
      id: v.id,
      registration: v.registration_no,
      model: v.models ? `${v.models.brand} ${v.models.name}` : '—',
      category: v.models?.categories?.name || '—',
    }));
...
const handleUpdate = async (id: string, updates: Partial<Pick<ReminderDB, 'type' | 'priority' | 'dueDate' | 'notes'>>) => {
  ...
  const handleComplete = async (id: string) => {
    ...
    if (error) { toast.error('Failed to update reminder.'); return; }
    setReminders(prev => prev.map(r => r.id === id ? { ...r, status: 'Completed', daysRemaining: 0 } : r));
    setIsDetailsOpen(false);
    toast.success('Reminder marked as completed!', { icon: '✅', style: { borderRadius: '16px', background: '#10B981', color: '#fff', fontWeight: 'bold' } });
  };
  ...
  const handleSnooze = async (id: string, days: number) => {
    ...
    const newDaysRemaining = reminder.daysRemaining + days;
    ...
    setIsDetailsOpen(false);
    toast.success(`Snoozed for ${days} days`, { icon: '⏰', style: { borderRadius: '16px', background: '#6360DF', color: '#fff', fontWeight: 'bold' } });
  };

  const handleDelete = async (id: string) => {
    const { error } = await supabase.from('reminders').delete().eq('id', id);
    if (error) { toast.error('Failed to delete reminder.'); return; }
    setReminders(prev => prev.filter(r => r.id !== id));
    setIsDetailsOpen(false);
    toast.error('Reminder deleted', { style: { borderRadius: '16px', background: '#EF4444', color: '#fff', fontWeight: 'bold' } });
  };
