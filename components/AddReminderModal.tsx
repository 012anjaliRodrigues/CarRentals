...
interface AddReminderModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (reminder: Omit<Reminder, 'id' | 'status' | 'daysRemaining'>) => void;
}
...
const vehicles = [
  { group: 'Hatchbacks', items: ['GA-01-AB-1234 • Swift', 'GA-02-CD-5678 • i20'] },
  { group: 'SUVs', items: ['GA-03-EF-9012 • Nexon', 'GA-04-GH-3456 • Creta'] },
  { group: 'Sedans', items: ['GA-05-IJ-7890 • City', 'GA-06-KL-1234 • Verna'] }
];

const drivers = ['Rajesh Kumar', 'Amit Singh', 'Vikram Patil', 'Suresh Raina', 'Self (Admin)'];
...
const AddReminderModal: React.FC<AddReminderModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.vehicle || !formData.type || !formData.dueDate) return;
    
    const [plate, model] = formData.vehicle.split(' • ');
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              ...
              <div className="space-y-2 relative">
                ...
                >
                  <span className={formData.vehicle ? 'text-[#151a3c]' : 'text-[#6c7e96]'}>
                    {formData.vehicle || 'Search & Select Vehicle'}
                  </span>
                  <ChevronDown size={18} className={cn("transition-transform", isVehicleDropdownOpen && "rotate-180")} />
                </div>
                          ...
                          <div key={group.group}>
                            ...
                            {group.items.map((item) => (
                              <div 
                                key={item}
                                onClick={() => {
                                  setFormData({ ...formData, vehicle: item });
              ...
              <div className="space-y-2 relative">
                ...
                <div 
                  ...
                  className="w-full bg-[#f8f7ff] border border-[#d1d0eb] rounded-xl py-3.5 px-4 text-sm font-bold text-[#151a3c] flex items-center justify-between cursor-pointer hover:border-[#6360DF] transition-all"
                >
                  <span className={formData.type ? 'text-[#151a3c]' : 'text-[#6c7e96]'}>
                    {formData.type || 'Select Reminder Type'}
                  </span>
                  <ChevronDown size={18} className={cn("transition-transform", isTypeDropdownOpen && "rotate-180")} />
                </div>
                          ...
                          <div key={group.group}>
                            ...
                            {group.items.map((item) => (
                              <div 
                                key={item.label}
                                onClick={() => {
                                  setFormData({ ...formData, type: item.label, category: group.group as any });
                              ...
                              >
                                <div className="text-[#6360DF]">{item.icon}</div>
            ...
            <div className="px-8 py-6 border-t border-slate-100 flex items-center justify-end space-x-4 shrink-0">
              ...
              <button 
                ...
                disabled={!formData.vehicle || !formData.type || !formData.dueDate}
